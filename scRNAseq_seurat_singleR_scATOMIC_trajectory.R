...
# This function applies a minimalist theme and manually adds UMAP axis arrows.
# This approach bypasses the problematic 'margin' calls in theme_blank and provides a robust alternative.
add_umap_arrows_and_theme_void <- function(base_plot, seurat_obj_for_coords,
  ...
  custom_plot <- base_plot +
    ggplot2::theme_void() + # Explicitly use ggplot2::theme_void
    ggplot2::theme(         # Explicitly use ggplot2::theme
      plot.title = ggplot2::element_text(hjust = 0.5, face = "bold", size = 14), # Explicitly use ggplot2::element_text
      legend.position = if (show_legend) "right" else "none",
      legend.box.just = "left",
      legend.direction = "vertical",
      legend.key.size = grid::unit(10, "pt"), # Explicitly use grid::unit
      legend.background = ggplot2::element_blank(), # Explicitly use ggplot2::element_blank
      # Adjust plot margins to make space for arrows that extend outside panel
      plot.margin = ggplot2::margin(t = 5, r = 5, b = 15, l = 15, unit = "pt") # Explicitly use ggplot2::margin
    ) +
    # Add UMAP1 arrow and label
...
perform_scatomic_malignancy_detection <- function(seurat_obj) {
  ...
  input_matrix <- GetAssayData(seurat_obj, assay = "RNA", layer = "counts") # Using layer instead of slot
  ...
  # Add scATOMIC results to Seurat object metadata
  ...
  # Extract predictions and sanitize to prevent 'source(...)' strings
  predictions_for_seurat_raw <- final_scatomic_annotations[colnames(seurat_obj_subsetted), "scATOMIC_pred"]
  
  # FIX: Sanitize scATOMIC_prediction to remove problematic strings
  ...
  # FIX: Ensure problematic string is not a factor level if it somehow made it through
  ...
  # Generate UMAP with scATOMIC predicted malignancy/cell types using custom theme
  ...
  plot_scatomic_umap <- DimPlot(seurat_obj_subsetted,
                                ...
                                repel = TRUE,
                                pt.size = 0.5) +
    ggplot2::ggtitle("UMAP by scATOMIC Predicted Cell Types") + # Explicit ggplot2 call
    ggplot2::guides(color = ggplot2::guide_legend(override.aes = list(size = 3), ncol = 1)) # Explicit ggplot2 call
  ...
  # Return the subsetted object that contains the scATOMIC annotations
  return(seurat_obj_subsetted)
}

# --- SCP-like Plotting Functions (from SCP_plots.R) ---

# Helper functions originally from SCP_plots.R
`%||%` <- function(a, b) {
...
CellDimPlot <- function(srt, group.by, reduction = NULL, dims = c(1, 2), split.by = NULL, cells = NULL,
  ...
  # Handle split.by for grouping (if not already defined, make a dummy one)
  if (is.null(split.by)) {
    split.by <- "All.groups"
    srt@meta.data[[split.by]] <- factor("")
  }
  
  # Ensure all necessary metadata columns exist and are factors
  ...
  dat_meta_orig <- srt@meta.data[, unique(c(group.by, split.by, stat.by)), drop = FALSE] # Include stat.by for merge
  ...
  # Generate a consistent color palette for the stat.by variable (used for pie charts)
  stat_by_levels <- if (!is.null(stat.by)) levels(dat_use[[stat.by]]) else NULL
  stat_by_colors <- if (!is.null(stat.by)) palette_scp(stat_by_levels, palette = "Set1", type = "discrete") else NULL # Set1 for distinct phases/subtypes
  
  # Main plotting loop for each split/group combination
  plist <- lapply(setNames(rownames(comb), rownames(comb)), function(i) {
    ...
    # Filter data for current split group
    if (s != "") { # Only filter if split.by is not the dummy "All.groups"
      cells_mask <- dat_filtered[[split.by]] != s
      dat_filtered[[g]][cells_mask] <- NA # Hide points not in current split
      if (!is.null(stat.by)) dat_filtered[[stat.by]][cells_mask] <- NA # Also hide stat.by data
    }
    
    # Remove rows with NA in the main grouping variable (g)
    ...
    p <- ggplot2::ggplot(dat_filtered, ggplot2::aes(x = .data[[paste0(reduction_key, dims[1])]], y = .data[[paste0(reduction_key, dims[2])]], color = .data[[g]])) +
      ...
      ggplot2::theme(
        ...
        legend.position = legend.position, # Corrected line,
        legend.box.just = "left",
        legend.direction = "vertical",
    ...
    # Main cluster labels and centroids for pie charts
    ...
    # Add cluster labels with ggrepel
    if (isTRUE(label)) {
      if (isTRUE(show_stat)) {
    ...
    # Add stat.by pie charts if requested and labels are active
    if (!is.null(stat.by) && isTRUE(label)) {
      ...
      # Get UMAP axis ranges to scale the pie chart size
      ...
      # Determine an appropriate size factor for the pie charts based on overall UMAP scale
      # The stat_plot_size is a proportion of the plot's range
      size_factor_x <- umap_range_x * stat_plot_size # horizontal size of pie chart
      size_factor_y <- umap_range_y * stat_plot_size # vertical size of pie chart
      ...
      for (cluster_label in unique(pie_data_per_cluster[[g]])) {
        current_cluster_data <- pie_data_per_cluster %>%
          ...
          dplyr::arrange(.data[[stat.by]]) # Ensure consistent slice order
        
        # Get the centroid coordinates for this cluster
        centroid <- label_df %>%
          dplyr::filter(.data[["label"]] == cluster_label) %>%
          dplyr::select(x, y)
        
        if (nrow(centroid) == 0) next # Skip if no centroid (e.g., cluster was filtered out)
        ...
        pie_chart <- ggplot(current_cluster_data, aes(x = 1, y = percentage, fill = .data[[stat.by]])) +
          ...
          scale_fill_manual(values = stat_by_colors[levels(current_cluster_data[[stat.by]])], drop = FALSE) + # Use all defined levels
          theme_void() +
          theme(
            legend.position = "none",
            plot.margin = ggplot2::margin(0,0,0,0) # No extra margins for grob
          )
        
        # Convert to grob
        pie_grob <- ggplot2::ggplotGrob(pie_chart)
        
        # Add grob to the main plot at the cluster's centroid
      ...
      # Create a separate legend for the stat.by variable
      stat_legend_plot <- ggplot(data.frame(var = stat_by_levels, dummy = 1), aes(x = dummy, y = var, fill = var)) +
        geom_tile() + # Simple rectangle for legend keys
    ...
    # If there are additional legends (like the stat.by legend), combine them
...
FeatureDimPlot <- function(srt, variable, reduction = NULL, dims = c(1, 2), split.by = NULL, cells = NULL,
                           show_na = FALSE, show_stat = FALSE, # show_stat set to FALSE by default here to simplify
  ...
  plist <- lapply(setNames(rownames(comb), rownames(comb)), function(i) {
    ...
    feature_data_for_plotting <- as.numeric(dat[[f]]) # Ensure numeric for palette_scp
...
# Adjusted add_umap_arrows_and_theme_void for SCP functions
# Note: SCP functions already add a theme, so this function is simplified for SCP plots
# to only add arrows if needed. The primary add_umap_arrows_and_theme_void at the top
# of the script is generally preferred for consistency.
# For the SCP plots section, we will use a version that adapts.
add_umap_arrows_for_scp <- function(plot, srt_object, reduction_name, show_legend = TRUE) {
  # This function adapts the main add_umap_arrows_and_theme_void to be used with SCP plots
  # that already have a theme applied by CellDimPlot/FeatureDimPlot.
  # We extract existing limits and add arrows.
  ...
  # Get current plot limits to place arrows relative to the plot area
  # This is a bit tricky with ggplot, so we'll approximate based on the data range
  ...
  plot_with_arrows <- plot +
    ...
    # Ensure the plot limits are expanded to include arrows
    ggplot2::coord_cartesian(xlim = c(x_min - range_x * 0.05, x_max + range_x * 0.05),
                             ylim = c(y_min - range_y * 0.05, y_max + range_y * 0.05),
                             expand = FALSE,
                             clip = "off") +
    ggplot2::theme(legend.position = ifelse(show_legend, "right", "none")) # Ensure legend position is respected
...
if (length(saved_seurat_files) > 0) {
  ...
  while (!choice_made) {
    ...
    if (user_choice_file == 0) {
      ...
      message("8: Malignancy Detection (scATOMIC Classifier)")
      message("9: Cell Cycle Scoring")
      message("10: Generate SCP-like Cell Cycle Plots")
      message("11: Interactive Gene Expression Analysis & Visualization")
      message("12: Trajectory inference and Pseudotime Analysis") # This is your Monocle3 step
      message("13: Final Data Saving") # This should be the last step
      message("-----------------------")
      
      user_choice_step <- readline(prompt = "From which step would you like to resume (1-13)? ") # Corrected prompt max step
      start_step <- as.integer(user_choice_step)
      
      if (is.na(start_step) || start_step < 1 || start_step > 13) { # Corrected max step
        warning("Invalid step. Enter a number between 1 and 13. Reloading object and re-asking.")
        seurat_object <- NULL
        next
      } else {
        step_names <- c(
          ...
          "Annotation_CAF", # 6
          "DE", # 7
          "scATOMIC_Detection", # 8
          "Cell_Cycle_Scoring", # 9
          "Generate_SCP_Plots", # 10
          "Interactive_Gene_Analysis", # 11
          "Trajectory_Inference", # 12 <-- NEW NAME AND INDEX
          "Final_Save" # 13 <-- NEW INDEX
        )
...
if (start_step <= 1) {
  perform_qc_and_filter <- function(seurat_obj, min_umis = 500, max_umis = 100000,
    ...
    seurat_obj <- SetAssayData(seurat_obj, layer = "counts", new.data = GetAssayData(seurat_obj, assay = "RNA", layer = "counts")[rowSums(GetAssayData(seurat_obj, assay = "RNA", layer = "counts") > 0) >= min_cells, ]) # Using layer instead of slot
...
if (start_step <= 4) {
  perform_visualization <- function(seurat_obj) {
    ...
    print(add_umap_arrows_and_theme_void(p_cluster, seurat_obj, show_legend = FALSE)) # Applying custom theme
    dev.off()
    message(paste0("UMAP plot by clusters saved to '", output_filename_cluster_umap, "'."))
    
    # UMAP colored by Sample (if 'orig.ident' exists)
    if ("orig.ident" %in% colnames(seurat_obj@meta.data)) {
      ...
      p_sample <- DimPlot(seurat_obj, reduction = "umap", group.by = "orig.ident", label = FALSE, repel = TRUE) +
        ...
        guides(color = guide_legend(override.aes = list(size = 3)))
      print(add_umap_arrows_and_theme_void(p_sample, seurat_obj, show_legend = TRUE)) # Applying custom theme
      dev.off()
...
# --- 8. Cell Type Annotation (SingleR) & CAF Identification ---
if (start_step <= 6) {
  perform_cell_type_annotation <- function(seurat_obj) {
    message("Performing cell type annotation using SingleR...")
    
    # Check for celldex references and download if not available
    ...
    # Example reference: Human Primary Cell Atlas Data
    # Other options: BlueprintEncodeData(), NovershternHematopoieticData(), MonacoImmuneData()
    ...
    predictions <- SingleR(test = sce_obj,
                           ref = hpca.se,
                           labels = hpca.se$label.main) # or hpca.se$label.fine for finer labels
    ...
    p_singler <- DimPlot(seurat_obj, reduction = "umap", group.by = "singler_labels", label = TRUE, repel = TRUE) +
      ...
      guides(color = guide_legend(override.aes = list(size = 3)))
    print(add_umap_arrows_and_theme_void(p_singler, seurat_obj, show_legend = TRUE)) # Applying custom theme
    dev.off()
    message(paste0("UMAP plot with SingleR cell types saved to '", output_filename_singler_umap, "'."))
    
    # Optional: Identify Cancer-Associated Fibroblasts (CAFs) based on markers
    message("Identifying Cancer-Associated Fibroblasts (CAFs) based on marker expression...")
    caf_markers <- c("ACTA2", "COL1A1", "DCN", "FAP", "PDGFRA", "THY1", "VIM") # Common CAF markers
    
    # Calculate average expression of CAF markers
    seurat_obj$CAF_score <- colMeans(x = GetAssayData(seurat_obj, assay = "RNA", layer = "data")[caf_markers, , drop = FALSE]) # Using layer instead of slot
    
    # Simple thresholding for CAF identification (can be refined)
    # Consider using a more sophisticated method or manual curation
    ...
    p_caf <- DimPlot(seurat_obj, reduction = "umap", group.by = "is_CAF", label = FALSE, cols = c("lightgrey", "darkgreen")) +
      ...
      guides(color = guide_legend(override.aes = list(size = 3)))
    print(add_umap_arrows_and_theme_void(p_caf, seurat_obj, show_legend = TRUE)) # Applying custom theme
    dev.off()
...
if (start_step <= 7) {
  perform_differential_expression <- function(seurat_obj) {
    ...
    # Example: Compare Cluster 0 to all other cells
    # This can be customized to compare any two groups (e.g., cell types, conditions)
    ...
    # Example: Volcano Plot for Cluster 0 DE genes
    # Customize axis limits and gene labels as needed
    ...
    # You can extend this to loop through all clusters or perform comparisons of interest
    message("Differential Expression analysis complete.")
    return(de_results_cluster_0) # Return example DE results
  }
...
if (start_step <= 7) { # Only save if DE analysis was performed or resumed from before/at this step
  message("\n--- INTERMEDIATE DATA SAVING (after Differential Expression) ---")
...
if (start_step <= 8) {
  # This step uses the function defined earlier
...
if (start_step <= 9) {
  ...
  # Load cell cycle gene lists from Seurat
  ...
  # ADD THESE LINES HERE TO ENSURE 'Phase' IS A FACTOR
  message("Ensuring 'Phase' metadata is a factor for consistent plotting...")
  phase_order <- c("G1", "S", "G2M") # Define desired order for phases
  ...
  if (user_regress_choice == "yes") {
    ...
    # ScaleData is typically run on all features. If you previously scaled only variable features,
    # you might want to re-run ScaleData on all features after this step.
    ...
    # After regression, it's often a good idea to re-run dimensionality reduction and clustering
    # to see the effect of removing cell cycle variation.
    ...
    if (user_re_run_dimred == "yes") {
      ...
      # Using default 30 dimensions, customize if needed
  ...
  # Generate UMAP plots with cell cycle phases (regular Seurat plots)
...
# --- 12. Generate SCP-like Cell Cycle Plots ---
if (start_step <= 10) {
  message("Generating UMAP plots with cell cycle phases using SCP-like functions...")
  
  # Ensure Cell Cycle Scoring (step 9) has been run, as these plots depend on 'Phase', 'S.Score', 'G2M.Score'
  if (!("Phase" %in% colnames(current_seurat_object@meta.data) &&
    ...
    stop("Cell cycle scores (Phase, S.Score, G2M.Score) not found in Seurat object metadata. Please run step 9 first.")
  }
  ...
  p_cc_phase_scp <- CellDimPlot(current_seurat_object,
                                group.by = "seurat_clusters", # Group by clusters
                                stat.by = "Phase",            # Show Phase distribution within clusters
                                reduction = "umap",
                                label = TRUE,                 # Label clusters
                                pt.size = 0.5,
                                stat_plot_size = 0.08,        # Adjust pie chart size as needed (proportion of plot range)
                                stat_plot_label = FALSE) +    # You can set this to TRUE to label pie slices
  ...
  interactive_gene_analysis <- function(current_seurat_object) {
    while (TRUE) {
      ...
      for (target_gene in target_genes) {
        ...
        print(add_umap_arrows_and_theme_void(p_feature, current_seurat_object, show_legend = TRUE)) # Applying custom theme
        ...
        # Binary (Positive/Negative) UMAP Plot based on a threshold
        ...
        if (nchar(threshold_value_input) > 0) {
          ...
          # FIX: Directly assign the named vector for gene_expression_status
          # This avoids the 'SetNames' function which was causing issues.
          ...
          names(gene_status_vector) <- colnames(current_seurat_object) # Ensure names match cell barcodes
          
          current_seurat_object <- AddMetaData(
            object = current_seurat_object,
            metadata = gene_status_vector, # Pass the pre-named vector directly
          ...
          print(add_umap_arrows_and_theme_void(p_binary, current_seurat_object, show_legend = TRUE)) # Applying custom theme
...
if (start_step <= 13) {
  ...
  # --- 15.1. Install Monocle3 and dependencies if not already installed ---
  if (!requireNamespace("BiocManager", quietly = TRUE)) {
    install.packages("BiocManager")
  }
  ...
  # --- 15.2. Preprocessing 'scATOMIC_prediction' for Monocle3 ---
  ...
  # --- 15.3. Convert Seurat object to Monocle3 cell_data_set (CDS) object ---
  ...
  # --- 15.4. Run Monocle3 Clustering ---
  message("Running Monocle3 clustering...")
  cds <- cluster_cells(cds, reduction_method = "UMAP", k = 20, resolution = 1e-6)
  message("Monocle3 clustering complete. Partitioning is now handled implicitly by learn_graph (use_partition=TRUE) or not needed for single graph (use_partition=FALSE).")
  
  
  # --- 15.5. Learn the graph and order cells ---
  message("Learning cell trajectory graph and ordering cells with Monocle3...")
  
  cds <- learn_graph(cds, use_partition = FALSE)
  
  message("Setting 'Normal Fibroblast' as the root node for trajectory inference based on user preference.")
  
  root_cells <- colData(cds) %>%
    as.data.frame() %>%
    filter(scATOMIC_prediction == "Normal Fibroblast") %>%
    rownames()
  
  if (length(root_cells) == 0) {
    warning("No cells found with 'Normal Fibroblast' label. Monocle3 will try to find a root automatically.")
  ...
  # --- 15.6. Visualize the trajectory ---
  message("Generating Monocle3 trajectory plot...")
  
  output_filename_monocle_traj <- "UMAP_Monocle3_Trajectory_NormalFibroblastRoot.pdf"
  ...
  plot_cells(cds,
             ...
             graph_label_size = 3) +
    ggplot2::ggtitle("Monocle3 Trajectory (Pseudotime - Normal Fibroblast Root)") +
  ...
  plot_cells(cds,
             ...
             label_branch_points = TRUE,
             graph_label_size = 3) +
    ggplot2::ggtitle("Monocle3 Trajectory (scATOMIC Prediction - Normal Fibroblast Root)") +
    ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5, face = "bold"))
  
  dev.off()
  message(paste0("Monocle3 trajectory plots saved to '", output_filename_monocle_traj, "'."))
  
  saveRDS(cds, "monocle3_cds_object_NormalFibroblastRoot.rds")
  message("Monocle3 CDS object saved as 'monocle3_cds_object_NormalFibroblastRoot.rds'.")
