...
if (length(saved_seurat_files) > 0) {
  ...
  while (!choice_made) {
    ...
    if (user_choice_file == 0) {
      ...
      message("10: Generate SCP-like Cell Cycle Plots") # NEW STEP
      message("11: Interactive Gene Expression Analysis & Visualization") # RENAMED FROM 10 TO 11
      message("12: Final Data Saving") # RENAMED FROM 11 TO 12
      message("-----------------------")
      
      user_choice_step <- readline(prompt = "From which step would you like to resume (1-12)? ") # Updated max step
      start_step <- as.integer(user_choice_step)
      
      if (is.na(start_step) || start_step < 1 || start_step > 12) { # Updated max step
        warning("Invalid step. Enter a number between 1 and 12. Reloading object and re-asking.")
        seurat_object <- NULL
        next
      } else {
        step_names <- c("QC", "Normalization", "DimRed", "Visualization", "Markers", "Annotation_CAF", "DE", "scATOMIC_Detection", "Cell_Cycle_Scoring", "Generate_SCP_Plots", "Interactive_Gene_Analysis", "Final_Save") # Updated step names
