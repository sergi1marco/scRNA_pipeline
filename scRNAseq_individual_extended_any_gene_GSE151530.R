...
# =============================================================================
# DATA LOADING FUNCTION (with GSE151530 "C" sample filtering)
# =============================================================================

load_data <- function(data_path) {
  # If a directory is provided, auto-select a data file
  ...
  # --- GSE151530 detection and info file parsing ---
  is_gse151530 <- grepl("GSE151530", data_path, ignore.case = TRUE)
  info_file <- NULL
  c_samples <- NULL
  if (is_gse151530) {
    info_candidates <- list.files(dirname(data_path), pattern = "info", full.names = TRUE, ignore.case = TRUE)
    if (length(info_candidates) > 0) {
      info_file <- info_candidates[1]
      info_df <- read.delim(info_file, header = TRUE, stringsAsFactors = FALSE)
      # Extract "C" samples (Sample column starts with "C" and two digits)
      c_samples <- info_df$Sample[grepl("^C\\d{2}$", info_df$Sample)]
      if (length(c_samples) == 0) stop("No 'C' samples found in info file!")
      cat("Filtering for C samples:", paste(c_samples, collapse = ", "), "\n")
    } else {
      stop("GSE151530 detected but no 'info' file found.")
    }
  }
  ...
  if (grepl("\\.h5$", data_path, ignore.case = TRUE)) {
    ...
    # If GSE151530 and C samples, filter columns
    if (!is.null(c_samples)) {
      # Try to match sample IDs in colnames
      keep_cols <- which(sapply(barcodes, function(bc) any(startsWith(bc, sample_prefixes))))
      if (length(keep_cols) == 0) stop("No columns matched the 'C' samples in the h5 data!")
    ...
    # Filter: Keep only barcodes that start with "C" (i.e., exclude "H" samples)
    keep_cols <- which(startsWith(barcodes, "C"))
    if (length(keep_cols) == 0) stop("No barcodes starting with 'C' found in the matrix data!")
    mtx <- mtx[, keep_cols, drop = FALSE]
