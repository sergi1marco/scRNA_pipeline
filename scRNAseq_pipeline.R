...
convert_gene_symbols <- function(seurat_obj) {
  ...
  # This prevents errors if data is already in SYMBOL format.
  
  # Sample a subset of genes to check their format for efficiency.
  sample_size <- min(100, length(stripped_genes)) # Check up to 100 genes
  sampled_genes <- sample(stripped_genes, sample_size)
  
  # Use a regular expression to identify common ENSEMBL ID patterns (e.g., ENSG, ENST followed by numbers).
  # This pattern checks for "ENS" followed by 'G' or 'T' and then digits.
  is_ensembl <- grepl("^ENS[GT]\\d+", sampled_genes)
  
  # If less than 80% of sampled genes look like ENSEMBL IDs, assume they are gene symbols
  ...
  # Replace rownames with mapped symbols
...
perform_scatomic_malignancy_detection <- function(seurat_obj) {
  ...
  # This part assumes 'add_umap_arrows_and_theme_void' is a function you've defined or sourced.
  plot_scatomic_umap <- add_umap_arrows_and_theme_void(plot_scatomic_umap, seurat_object_filtered)
  
  # Save the plot - THIS REQUIRES 'output_dir' to be defined externally!
...
if (length(saved_seurat_files) > 0) {
  ...
  while (!choice_made) {
    ...
    if (user_choice_file == 0) {
      ...
      message("15: Cell-Cell Communication Analysis (CellphoneDB)") # NEWLY INTEGRATED STEP
...
# --- 3. Gene Symbol Conversion (NEW STEP) ---
  ...
  perform_qc_and_filter <- function(seurat_obj, min_umis = 500, max_umis = 100000,
    ...
    seurat_obj <- subset(seurat_obj,
                         subset = nFeature_RNA > min_features &
                           nCount_RNA < max_umis & # Changed to nCount_RNA as max_umis implies total counts
                           percent.mt < max_percent_mt)
    
    # CORRECTED GENE FILTERING:
    # Identify genes (features) that are detected in at least 'min_cells'
    genes_to_keep <- names(which(rowSums(GetAssayData(seurat_obj, assay = "RNA", layer = "counts") > 0) >= min_cells))
    
    # Subset the Seurat object by these identified genes
...
if (current_pipeline_step_marker >= start_step_numeric) {
  ...
  if (user_regress_choice == "yes") {
    ...
    if (user_re_run_dimred == "yes") {
      ...
      # Line to add: Regenerate the normalized UMAP cell types plot
