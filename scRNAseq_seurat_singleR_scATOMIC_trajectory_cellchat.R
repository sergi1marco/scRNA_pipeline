...
if (start_step <= 13) { # Step changed from 12 to 13
  ...
  unique_scatomic_labels <- levels(current_seurat_object$scATOMIC_prediction)
  ...
  if (tolower(selected_root_label) == "auto") {
    ...
    root_cells <- colData(cds) %>%
      as.data.frame() %>%
      filter(scATOMIC_prediction == selected_root_label) %>%
      rownames()
  ...
  saveRDS(cds, paste0("monocle3_cds_object_", root_label_for_filename, "Root.rds"))
  message(paste0("Monocle3 CDS object saved as 'monocle3_cds_object_", root_label_for_filename, "Root.rds'."))
...
if (start_step <= 14) {
  ...
  message("\nStarting Cell-Cell Communication Analysis using CellChat...")
  
  # Ensure CellChat and other necessary libraries are loaded
  library(CellChat)
  library(patchwork)
  library(ggplot2)
  library(dplyr)
  message("CellChat and its dependencies loaded successfully.")
  
  message("Preparing input data for CellChat...")
  
  if (!is.factor(current_seurat_object$scATOMIC_prediction)) {
    warning("'scATOMIC_prediction' is not a factor. Converting now.")
    current_seurat_object$scATOMIC_prediction <- factor(current_seurat_object$scATOMIC_prediction)
  }
  
  # Extract data
  data.input.normalized <- GetAssayData(current_seurat_object, assay = "RNA", slot = "data")
  data.input.raw_counts <- GetAssayData(current_seurat_object, assay = "RNA", slot = "counts")
  
  meta <- current_seurat_object@meta.data
  cell_type_column <- "scATOMIC_prediction"
  
  # Prompt for organism and load appropriate DB
  organism_choice <- tolower(readline(prompt = "Is your data from 'human' or 'mouse'? (Enter 'human' or 'mouse'): "))
  if (organism_choice == "human") {
    CellChatDB <- CellChatDB.human
  } else if (organism_choice == "mouse") {
    CellChatDB <- CellChatDB.mouse
  } else {
    stop("Invalid organism choice. Please enter 'human' or 'mouse'.")
  }
  
  # Attempt to match gene names
  signaling_genes <- unique(unlist(CellChatDB$interaction$genesymbol))
  original_genes <- rownames(data.input.normalized)
  
  matched_genes <- intersect(original_genes, signaling_genes)
  
  # If no match, try converting from Ensembl IDs to HGNC symbols
  if (length(matched_genes) == 0 && grepl("^ENSG", original_genes[1])) {
    message("Gene symbols appear to be Ensembl IDs. Attempting conversion to HGNC symbols...")
    
    gene_map <- convert_ensembl_to_symbols(original_genes)
    converted_genes <- gene_map[rownames(data.input.normalized), "hgnc_symbol"]
    names(converted_genes) <- rownames(data.input.normalized)
    
    # Replace rownames with gene symbols (drop empty)
    valid_genes <- which(!is.na(converted_genes) & converted_genes != "")
    data.input.normalized <- data.input.normalized[valid_genes, ]
    data.input.raw_counts <- data.input.raw_counts[valid_genes, ]
    rownames(data.input.normalized) <- converted_genes[valid_genes]
    rownames(data.input.raw_counts) <- converted_genes[valid_genes]
    
    matched_genes <- intersect(rownames(data.input.normalized), signaling_genes)
  }
  
  if (length(matched_genes) == 0) {
    stop("No matching signaling genes found in expression data after conversion. Cannot proceed.")
  }
  
  # Subset to signaling genes
  data.input.normalized <- data.input.normalized[matched_genes, ]
  data.input.raw_counts <- data.input.raw_counts[matched_genes, ]
  
  message(length(matched_genes), " signaling genes retained for CellChat analysis.")
  
  
  # Create CellChat object
  cellchat <- createCellChat(object = data.input.normalized, meta = meta, group.by = cell_type_column)
  message("CellChat object created.")
  cellchat@DB <- CellChatDB
  message(paste0("CellChat database set to ", toupper(organism_choice), "."))
  
  message("Preprocessing CellChat object...")
  cellchat <- subsetData(cellchat)
  
  if (is.null(cellchat@data.signaling) || nrow(cellchat@data.signaling) == 0) {
    stop("ERROR: 'object@data.signaling' is empty after subsetData. This usually means no signaling genes from the database were found in your expression data, or all were filtered out. Cannot proceed with communication analysis.")
  }
  
  message(paste0("Dimensions of subsetted expression data (object@data.signaling): ", paste(dim(cellchat@data.signaling), collapse = " x ")))
  
  message("Inferring cell-cell communication probabilities and pathways...")
  cellchat <- computeCommunProb(cellchat, type = "truncatedMean", trim = 0.1)  # FIXED: removed unsupported args
  cellchat <- filterCommunication(cellchat, min.cells = 10)
  
  if (is.null(cellchat@netP$prob) || all(is.na(cellchat@netP$prob)) || sum(cellchat@netP$prob, na.rm = TRUE) == 0) {
    stop("ERROR: No significant cell-cell communication probabilities were inferred after filtering.
          This could be due to:
          1. Very stringent 'min.cells' threshold.
          2. Low expression of detected ligand-receptor pairs.
          3. Lack of strong signaling interactions in your dataset.")
  }
  
  cellchat <- computeCommunProbPathway(cellchat)
  message("Cell-cell communication inference complete.")
  
  message("Analyzing and visualizing communication networks...")
  df.net <- subsetCommunication(cellchat)
  message(paste0("Total inferred interactions after filtering: ", nrow(df.net)))
  
  output_filename_overall_net <- "CellChat_Overall_Communication_Network.pdf"
  pdf(output_filename_overall_net, width = 10, height = 8)
  netVisual_circle(cellchat@net$count, vertex.weight = as.numeric(table(cellchat@idents)),
                   weight.scale = TRUE, label.edge = FALSE,
                   title.name = paste0("Overall Cell-Cell Communication Network (", toupper(organism_choice), ")"))
  dev.off()
  message(paste0("Overall communication network saved to '", output_filename_overall_net, "'."))
  
  cellchat <- netAnalysis_signalingRoles(cellchat, slot.name = "netP")
  output_filename_roles_heatmap <- "CellChat_Signaling_Roles_Heatmap.pdf"
  pdf(output_filename_roles_heatmap, width = 12, height = 8)
  netVisual_analyzer(cellchat, pattern = "outgoing", font.size = 10) +
    ggtitle("Outgoing Signaling Roles (Pathways)")
  netVisual_analyzer(cellchat, pattern = "incoming", font.size = 10) +
    ggtitle("Incoming Signaling Roles (Pathways)")
  dev.off()
  message(paste0("Signaling roles heatmaps saved to '", output_filename_roles_heatmap, "'."))
  
  # Optional pathway-specific visualizations (customize below if needed)
  # pathways.show <- c("MIF", "COLLAGEN")
  # if (all(pathways.show %in% cellchat@netP$pathways)) {
  #   pdf("CellChat_Specific_Pathways.pdf", width = 12, height = 8)
  #   for (pathway_name in pathways.show) {
  #     netVisual_bubble(cellchat, signaling = pathway_name,
  #                      title.name = paste0(pathway_name, " signaling in ", toupper(organism_choice)))
  #   }
  #   dev.off()
  # }
  
  output_filename_cellchat_obj <- "cellchat_object.rds"
  saveRDS(cellchat, output_filename_cellchat_obj)
  message(paste0("CellChat object saved as '", output_filename_cellchat_obj, "'."))
  
  message("Cell-Cell Communication Analysis step completed.")
}
