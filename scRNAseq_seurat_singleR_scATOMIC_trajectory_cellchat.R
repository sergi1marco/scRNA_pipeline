...
if (start_step <= 14) {
  ...
  pdf(file.path(data_dir, "cellchat_netAnalysis_signalingRole_heatmap.pdf"), width = 8, height = 6)
  print(netAnalysis_signalingRole_heatmap(cellchat))
  dev.off()
  
  pdf(file.path(data_dir, "cellchat_netAnalysis_signalingRole_scatter.pdf"), width = 8, height = 6)
  print(netAnalysis_signalingRole_scatter(cellchat))
  dev.off()
  
  # --- Optional: Pathway selection ---
  ...
  if (selected_pathway %in% cellchat@netP$pathways) {
    pdf(file.path(data_dir, paste0("cellchat_netVisual_bubble_", selected_pathway, ".pdf")), width = 8, height = 6)
    print(netVisual_bubble(cellchat, signaling = selected_pathway))
    dev.off()
    
    pdf(file.path(data_dir, paste0("cellchat_netVisual_chord_", selected_pathway, ".pdf")), width = 8, height = 6)
    netVisual_chord_gene(cellchat, signaling = selected_pathway)
    dev.off()
